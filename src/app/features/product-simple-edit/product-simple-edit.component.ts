...
@Component({
  selector: 'app-product-simple-edit',
  imports: [CommonModule, FontAwesomeModule, ReactiveFormsModule, BrandsComponent, CategoryComponent],
  providers: [DatePipe],
  templateUrl: './product-simple-edit.component.html',
  styleUrl: './product-simple-edit.component.scss'
})
export class ProductSimpleEditComponent implements OnInit, AfterViewInit {
  ...
  constructor(private route: ActivatedRoute,
    ...
    // private WarehouseService: WarehousService,
  ...
  ngOnInit(): void {
    ...
    if (this.showImageFlag) {
      ...
      this.productService.getProductsById(productId).subscribe((data: ProductView) => {
        ...
        this.uploadService.getImageFilesDB(this.editProduct.productId).subscribe(data => {
  ...
  ngAfterViewInit() {
    this.zone.runOutsideAngular(() => {
      // Bootstrap JS is already listening — we just need Angular to detect DOM changes
      // Sometimes just having the script is enough, but if not:
      const tabList = document.querySelectorAll('.nav-link[data-bs-toggle="tab"]');
      tabList.forEach(tab => {
  ...
  convertProductToForm(product: ProductView) {
    ...
    this.productForm.get('hsn')?.setValue(product.hsn);
  ...
  convertFormToProduct(): Product {
    ...
    product.productDetails = this.productForm.get('productDetails')?.value === undefined ? product.productName : this.productForm.get('productDetails')?.value;
    product.categoryId = this.productForm.get('categoryId')?.value;
    product.discount = this.productForm.get('discount')?.value === null ? 0 : this.productForm.get('discount')?.value;
    product.unitPrice = this.productForm.get('unitPrice')?.value === null ? 0 : this.productForm.get('unitPrice')?.value;
    ...
    product.madein = this.productForm.get('madein')?.value === null ? '' : this.productForm.get('madein')?.value;
    product.instockFlag = (this.productForm.get('instockFlag')?.value ? 1 : 0);
    product.sellinPcs = (this.productForm.get('sellinPcs')?.value ? 1 : 0);
    product.wholesalePrice = this.productForm.get('wholesalePrice')?.value === null ? 0 : this.productForm.get('wholesalePrice')?.value;
    product.quantity = this.productForm.get('quantity')?.value === null ? 0 : this.productForm.get('quantity')?.value;
    product.physicalDimension = this.productForm.get('physicalDimension')?.value;
    product.weight = this.productForm.get('weight')?.value === null ? 0 : this.productForm.get('weight')?.value;
    product.purchasePrice = this.productForm.get('purchasePrice')?.value === null ? 0 : this.productForm.get('purchasePrice')?.value;
    product.salePrice = this.productForm.get('salePrice')?.value === null ? 0 : this.productForm.get('salePrice')?.value;
    ...
    product.tax = this.productForm.get('tax')?.value === false ? 0 : this.productForm.get('tax')?.value;
    product.hsn = this.productForm.get('hsn')?.value;

    //Added on Feb 25, 2024
    product.qtyCase = this.productForm.get('qtyCase')?.value === null ? 0 : this.productForm.get('qtyCase')?.value;
    ...
    //Bug Fixed on Jan 20, 2026. To keep the image in this.editProduct.productImage
    if (this.editProduct.productImage !== null || this.editProduct.productImage !== undefined) {
      product.productImage = this.editProduct.productImage;
      product.imageMimeType = this.editProduct.imageMimeType;
    }
  ...
  selectFile(event: any): void {
    ...
    let removePreviewButton = <HTMLButtonElement>(document.getElementById('removePreviewButton'));
    if (removePreviewButton !== null) {
      removePreviewButton.removeAttribute('hidden');
    }
  ...
  removePreviewImage() {
    let removePreviewButton = <HTMLButtonElement>(document.getElementById('removePreviewButton'));
    removePreviewButton.setAttribute('hidden', 'hidden');
  ...
  removeImage(productId: any, imageId: any) {
    ...
    //CASE-2: One Image was saved in Products table but not in ProductImage Table
    else if (imageId < 0) {
      //No image in ProductImage but in Products Table
      let buttonElementId = 'myRemoveButton-' + productId;
      let myRemoveButton = <HTMLButtonElement>(document.getElementById(buttonElementId));
      myRemoveButton.setAttribute('hidden', 'hidden');
      this.editProduct.productImage = null;
      this.editProduct.imageMimeType = null;
    ...
    //CASE-3: Multiple Images was saved in ProductImage Table
    else {
